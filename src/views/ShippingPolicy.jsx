...
import { getShippingPolicyContent } from "@/data/shipping-policy-content";
    ...
    <div className="overflow-hidden rounded border border-gray-200" role="group" aria-label={label}>
      <dl className="divide-y divide-gray-200">
        {items.map(([term, description]) => (
          <div key={term} className="grid gap-1 bg-white px-4 py-3 sm:grid-cols-[200px_minmax(0,1fr)] sm:gap-5 sm:px-5">
            <dt className="min-w-0 font-semibold text-navy-800">{term}</dt>
            <dd className="min-w-0 text-gray-600">{description}</dd>
          </div>
        ))}
      </dl>
    </div>
...
function Notice({ tone, text, email = false }) {
  ...
  return (
    <div className={`flex gap-3 border-l-4 px-5 py-4 ${style.wrapper}`}>
      ...
      <p className={`min-w-0 font-semibold ${style.text}`}>
        {text}
        {email && (
          <>
            {" "}
    ...
    return (
      <div className="border border-gray-200 bg-gray-50 px-5 py-6 sm:px-6">
        <div className="mb-5 flex items-center gap-3">
          <PackageCheck className="h-6 w-6 shrink-0 text-orange-500" />
          <h3 className="font-bold text-navy-800">{copy.needHelp}</h3>
        </div>
...
export default function ShippingPolicy() {
  ...
  const copy = getShippingPolicyContent(language);
